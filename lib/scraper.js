...
cleanCategory(category) {
    ...
    // Add this method to your HiruNewsScraper class in scraper.js
async getNewsByCategory(categoryName, limit = 15) {
    console.log(`📂 Fetching category: ${categoryName}`);
    
    // Map category names to actual URLs
    const categoryUrls = {
        'sports': `${this.baseUrl}/news_listing.php?category=Sports`,
        'business': `${this.baseUrl}/news_listing.php?category=Business`,
        'entertainment': `${this.baseUrl}/news_listing.php?category=Entertainment`,
        'international': `${this.baseUrl}/news_listing.php?category=International`,
        'general': `${this.baseUrl}/news_listing.php?category=General`,
        'local': `${this.baseUrl}/news_listing.php?category=General`
    };
    
    const url = categoryUrls[categoryName.toLowerCase()];
    ...
    if (!$) {
        console.log(`❌ Failed to fetch category page: ${url}`);
        return [];
    }
    
    const articles = [];
    
    // METHOD 1: Look for articles in category listing
    ...
    for (const selector of selectors) {
        $(selector).each((i, el) => {
            ...
            const $el = $(el);
            const url = $el.attr('href');
            const headline = $el.find('h3, h4, .title, .card-title-v3, .card-title-v2, .card-title-v1').text().trim() ||
                           $el.text().trim();
            const thumbnail = $el.find('img').attr('src');
            
            // Validate it's a news article link
            if (url && headline && 
                (url.includes('/news/') || url.match(/\/\d{6}\//)) &&
                headline.length > 10 &&
                ...
                if (articleId && !articles.some(a => a.id === articleId)) {
                    articles.push({
                        ...
                        timestamp: new Date().toISOString(),
                        basic: true // Mark as needing full fetch
                    });
                }
            }
        });
    ...
    console.log(`✅ Found ${articles.length} articles in ${categoryName} category`);
    
    // If no articles found with above methods, try generic approach
    ...
    // Fetch full details for first few articles
    const detailedArticles = [];
    const maxToFetch = Math.min(articles.length, 5); // Limit to 5 for speed
    
    for (let i = 0; i < maxToFetch; i++) {
        const article = articles[i];
        if (article.id) {
            try {
                console.log(`📄 Fetching full article ${i + 1}/${maxToFetch}: ${article.id}`);
                ...
                console.log(`⚠️ Failed to fetch full article ${article.id}: ${error.message}`);
                detailedArticles.push(article);
            }
        }
    }
    
    // Add remaining articles without full details
    for (let i = maxToFetch; i < articles.length; i++) {
        detailedArticles.push(articles[i]);
    }
